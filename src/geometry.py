"""
geometry operations, modified from Badger et al.
...
def perspective_projection_homo(points, proj_m):
    ...
    Input:
        points (bs, N, 3): 3D points
        proj_m (bs, 4, 4): perspective projection matrix without distortion

    Output:
        projected_points (bs, N, 2)
    """

    homogeneous_points = torch.cat([points, torch.ones(points.size(0), points.size(1), 1)], -1)

    out_points = []
    for i in range(proj_m.size(0)):
        # p' = PI * p
        projected_points = torch.matmul(proj_m[i], homogeneous_points[i].permute(1,0)).permute(1,0)
        projected_points = torch.div(projected_points , torch.stack([projected_points[:,-1]] * 4, -1))
        out_points.append(projected_points)
...
def perspective_projection_ref(points, rotation, translation,
    ...
    Input:
        ...
        distortion (bs, 5): distortion factors [rad1, rad2, tan1, tan2, rad3]
    """
